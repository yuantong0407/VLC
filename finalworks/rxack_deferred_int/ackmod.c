...
// #define INADDR_TX  ((unsigned long int) 0xc0a80201U) //192.168.2.3 // for test
...
// const unsigned char TX_MAC[ETH_ALEN] = {0xE8, 0x03, 0x9A, 0xFB, 0x01, 0x72}; // Samsung WiFi
...
static void work_queue_init(void){
  my_wq = create_workqueue("my_queue");
  if(!my_wq){
      printk ("<1>work_queue_init: cannot create wq\n");
  }
}
...
static void my_wq_function( struct work_struct *work)
{
  ...
  ret = dma_receive(my_work->x);
  // printk("<1> dma_receive(%d) returned with %d", my_work->x, ret);
...
{      
 ...
 /* Queue some work (item 1) */
  ...
  if (work) {
    ...
    queue_work_on( 0, my_wq, (struct work_struct *)work );
  }
  else{
    printk ("<1>dma: cannot malloc work\n");
    return -1;
  }
...
{      
 ...
 /* Queue some work (item 1) */
  ...
  if (work1) {
    ...
    queue_work_on( 0, my_wq, (struct work_struct *)work1 );
  }
  else{
    printk ("<1>dma: cannot malloc work1\n");
    return -1;
  }
...
static int __init ack_init(void)
{
  ...
  printk("work_queue_init\n");
  work_queue_init();

  printk("@@ version v 4: ack/nak, to feiye's laptop computer.\n");
